...
• Today — next move and a ticket-style flight card
• Plan — the full schedule with dates, not only times
• Map — pins and native Apple Maps directions, with a clear Close control
• Pack — a list that stays on this iPhone
• Apps — opens Wizz Air, FlixBus, Airbnb, Booking, MOL Bubi, Bumble, Timeleft or Nomadtable if they are already installed
• Export — add the plan to Calendar or Reminders

Swipe between tabs. The header and dock stay on screen.
...
itinerary,travel,tickets,flight,bus,packing,maps,reminders,wizz,flixbus,airbnb,trip planner

## What's New (1.0)

First release. One on-device timeline for trips booked across several cheap-ticket apps. Sample dates: 14–21 September 2026.

## Review notes

This build is a private itinerary companion. There is no login. Reviewers can tap Load sample itinerary. Personal booking codes are not included. Deep links open third-party apps only when those apps are installed; otherwise the system handles the missing scheme.
...
- Source: https://github.com/CemalSakin/belvia
