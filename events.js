(function(){
  if(!document.getElementById("deskCss")){
    var l=document.createElement("link");
    l.id="deskCss";
    l.rel="stylesheet";
    l.href="./desk.css?v=mit2";
    document.head.appendChild(l);
  }
  var today=document.getElementById("page-today");
...
document.addEventListener("click", function(e){
  ...
  else if(act==="pack"){ const id=actEl.getAttribute("data-id"); S.packed[id]=!S.packed[id]; save(); actEl.classList.toggle("on", !!S.packed[id]); const k=$("packCount"); if(k){ const done=PACK.reduce((n,g)=>n+g[1].filter(i=>S.packed[i[0]]).length,0); const total=PACK.reduce((n,g)=>n+g[1].length,0); k.textContent=done+" / "+total+" "+(typeof t==="function"?t("packed"):""); } }
  ...
  else if(act==="wipe"){ if(confirm(typeof t==="function"?t("wipeQ"):"Clear?")){ S=emptyState(); save(); paintAll(); } }
  ...
  else if(act==="ics-cal"){ if(!S.reminders.some(r=>r.at)){ alert(typeof t==="function"?t("icsNeed"):"Add times first"); return; } downloadIcs("BudVia-Calendar.ics", buildIcs("event")); }
  else if(act==="ics-rem"){ if(!S.reminders.some(r=>r.at)){ alert(typeof t==="function"?t("icsNeed"):"Add times first"); return; } downloadIcs("BudVia-Reminders.ics", buildIcs("todo")); }
...
(function(){
  ...
  pager.addEventListener("touchend", function(e){ if(skip || window.matchMedia("(min-width:880px)").matches) return; const dx=e.changedTouches[0].clientX-x0, dy=e.changedTouches[0].clientY-y0; if(Math.abs(dx)<56 || Math.abs(dx)<Math.abs(dy)*1.25) return; const i=TABS.indexOf(tab); if(dx<0 && i<TABS.length-1) setTab(TABS[i+1]); if(dx>0 && i>0) setTab(TABS[i-1]); }, {passive:true});
})();
window.addEventListener("resize", function(){
  const pager=$("pager");
  if(!window.matchMedia("(min-width:880px)").matches && pager) pager.scrollLeft=TABS.indexOf(tab)*pager.clientWidth;
...
loadScript("./i18n.js?v=mit2").then(function(){
  return loadScript("./tickets.js?v=mit2");
}).then(function(){
  return loadScript("./install.js?v=mit2");
}).then(function(){
  paintAll();
  if(typeof setTab==="function") setTab(tab);
});
