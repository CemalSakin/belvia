document.addEventListener("click", function(e){
  ...
  else if(act==="pack"){ const id=actEl.getAttribute("data-id"); S.packed[id]=!S.packed[id]; save(); actEl.classList.toggle("on", !!S.packed[id]); const k=$("packCount"); if(k){ const done=PACK.reduce((n,g)=>n+g[1].filter(i=>S.packed[i[0]]).length,0); const total=PACK.reduce((n,g)=>n+g[1].length,0); k.textContent=done+" / "+total; } }
  ...
  else if(act==="wipe"){ if(confirm("Remove this trip from the phone?")){ S=emptyState(); save(); paintAll(); } }
...
(function(){
  ...
  let t; pager.addEventListener("scroll", function(){ clearTimeout(t); t=setTimeout(function(){ const w=pager.clientWidth||1; const i=Math.round(pager.scrollLeft/w); const id=TABS[Math.max(0,Math.min(TABS.length-1,i))]; if(id!==tab){ tab=id; document.querySelectorAll("#pills button, #dock button").forEach(b=>b.classList.toggle("on", b.getAttribute("data-go")===id)); if(id==="map") ensureMap(); } },50); }, {passive:true});
})();
