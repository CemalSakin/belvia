...
function teach(title, body){
  return '<div class="card example-card" style="margin-top:18px"><div class="pad"><p class="kicker">How this tab works</p><h3>'+title+'</h3><p class="muted">'+body+'</p></div></div>';
}
function paintPlan(){
  ...
  html+=teach("One row, one clock","Tick when done. Delete when you do not need it. Export writes the timed rows into Calendar.");
  $("page-plan").innerHTML=html;
}
function paintMap(){
  let html='<p class="kicker">Route</p><h2>Places</h2><p class="muted">A pin is a place you will stand.</p><div class="mapwrap card"><button class="mapclose" type="button" data-act="go" data-go="today">Close</button><div id="map"></div></div><div class="card" style="margin-top:12px">';
  if(!S.places.length) html+='<div class="pad"><p class="muted">No pins yet. Add one below, or load the sample trip.</p></div>';
  ...
  html+='<div class="card" style="margin-top:12px"><div class="pad"><p class="kicker">New pin</p><h3>Add a place</h3><p class="note">Name the spot the way a taxi driver hears it.</p></div>';
  html+='<div class="field"><label>Name</label><input id="pName" placeholder="BUD Terminal 2B" /></div>';
  html+='<div class="field"><label>Address</label><input id="pAddr" placeholder="Budapest Airport arrivals hall" /></div>';
  html+='<div class="pad"><button class="btn btn-a" type="button" data-act="add-place">Add place</button></div></div>';
  html+=teach("Directions open Maps","Phone uses Apple Maps. Desktop uses Google Maps. Delete a pin you will not stand on.");
...
function paintPack(){
  ...
  let html='<p class="kicker" id="packCount">'+done+' / '+total+' packed</p><h2>Bag</h2><p class="muted">Tick what is in the bag. Add the one odd item you always forget.</p><div class="stack">';
  ...
  html+='<div class="field" style="padding-top:12px"><label>Item</label><input id="packTitle" placeholder="EU plug, Type C / F" /></div>';
  html+='<div class="pad"><button class="btn btn-a" type="button" data-act="add-pack">Add bag item</button></div></div>';
  html+='</div><div class="card" style="margin-top:12px"><div class="field" style="padding-top:14px"><label>Bag notes</label><textarea id="extra" rows="3" placeholder="Power bank stays in the cabin bag, under 20,000 mAh.">'+esc(S.extra)+'</textarea></div></div>';
  html+=teach("Packed / total","The number at the top is the whole bag. Tick. Delete only the extras you added.");
  $("page-pack").innerHTML=html;
}
function paintApps(){
  $("page-apps").innerHTML=renderTicketBoard()+
    '<div class="card" style="margin-top:12px"><div class="pad"><p class="kicker">Contact</p><h3>BUD&VIA</h3><img class="example-shot" alt="BudVia otter" src="./0875CF7C-5ACB-48B1-B2CD-57E02C5C9B58.jpeg?v=en3"/><p class="note">An idiot admires complexity, a genius admires simplicity.</p><p class="note">\u2014 Terry A. Davis</p><p class="note">This copy never leaves the phone. No login. No analytics.</p><p class="note">Publisher: Tahsin Sakin.</p><p class="note"><a class="act" href="https://www.linkedin.com/in/tahsin-sakin-390961199" rel="noopener noreferrer" target="_blank">LinkedIn</a></p></div></div>';
}
